...
function createMap(atts, extensions = {}) {
  ...
  const map = {
    ...
    fontSet(name, faceNames) {
      const fsEle = mapEle.ele('FontSet', { name });
      for (const faceName of faceNames) {
        fsEle.ele('Font', { 'face-name': faceName });
      }
      return map;
    },
      ...
      const style = {
        ...
        rule({ filter, maxZoom, minZoom } = {}) {
          ...
          const rule = {
            ...
            shieldSymbolizer(atts = {}, text) {
              const shieldSymbolizerEle = ruleEle.ele('ShieldSymbolizer', sanitizeAtts(atts), text);
